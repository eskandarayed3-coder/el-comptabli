...
export default function TrialBalance() {
  ...
  const { t } = useT();
  const rows = (state.trialBalance || []).map((row) => ({
    num: row.account_number, label: row.label, debit: Number(row.total_debit || 0), credit: Number(row.total_credit || 0),
  }));
  ...
  return (
    <div className="screen stagger">
      <TopBar title={t('accounting.trial')} />
      ...
      {!rows.length && <p className="small muted center">Aucune écriture comptabilisée.</p>}
